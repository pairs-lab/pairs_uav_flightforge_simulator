...
tmux send-keys -t "$P" "$SETUP; "'export UAV_NAME=uav1; waitForTime; roslaunch pairs_uav_unreal_simulation hw_api.launch' Enter
P=$(tmux split-window -t "$W_hw_api" -P -F '#{pane_id}')
tmux select-layout -t "$W_hw_api" tiled
tmux send-keys -t "$P" "$SETUP; "'export UAV_NAME=uav2; waitForTime; roslaunch pairs_uav_unreal_simulation hw_api.launch' Enter
tmux select-layout -t "$W_hw_api" tiled
...
tmux send-keys -t "$P" "$SETUP; "'export UAV_NAME=uav1; waitForHw; sleep 3; roslaunch pairs_uav_core core.launch platform_config:=`rospack find pairs_multirotor_simulator`/config/pairs_uav_system/$UAV_TYPE.yaml custom_config:=./config/custom_config.yaml world_config:=./config/world_config.yaml network_config:=./config/network_config.yaml' Enter
P=$(tmux split-window -t "$W_control" -P -F '#{pane_id}')
tmux select-layout -t "$W_control" tiled
tmux send-keys -t "$P" "$SETUP; "'export UAV_NAME=uav2; waitForHw; sleep 3; roslaunch pairs_uav_core core.launch platform_config:=`rospack find pairs_multirotor_simulator`/config/pairs_uav_system/$UAV_TYPE.yaml custom_config:=./config/custom_config.yaml world_config:=./config/world_config.yaml network_config:=./config/network_config.yaml' Enter
tmux select-layout -t "$W_control" tiled
...
tmux send-keys -t "$P" "$SETUP; "'waitForRos; roslaunch pairs_tf_connector tf_connector.launch' Enter
