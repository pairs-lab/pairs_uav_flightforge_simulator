...
void Api::initialize(const ros::NodeHandle& parent_nh, std::shared_ptr<pairs_uav_hw_api::CommonHandlers_t> common_handlers) {
  ...
  shopts.nh                 = nh_;
  shopts.node_name          = "PairsSimulatorHwApi";
...
std::tuple<bool, std::string> Api::callbackArming([[maybe_unused]] const bool& request) {
  ...
  if (request) {
    ...
    ss << "armed";
    ROS_INFO_STREAM_THROTTLE(1.0, "[PairsSimulatorHwApi]: " << ss.str());
...
std::tuple<bool, std::string> Api::callbackOffboard(void) {
  ...
  if (!armed_) {
    ...
    ROS_INFO_THROTTLE(1.0, "[PairsSimulatorHwApi]: %s", ss.str().c_str());
    return {false, ss.str()};
  }
...
void Api::callbackOdom(const nav_msgs::Odometry::ConstPtr msg) {
  ...
  if (_capabilities_.produces_magnetometer_heading) {
    ...
    double heading = pairs_lib::AttitudeConverter(odom->pose.pose.orientation).getHeading();
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_unreal_simulation::Api, pairs_uav_hw_api::PairsUavHwApi)
